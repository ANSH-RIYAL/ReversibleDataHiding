...
def modeify(originalImage, n, Data):
    # Major Variables:
    ...
    modeImage = np.array(originalImage, dtype = np.int16)
    step = 2*n+1
    l,w = modeImage.shape
    a = np.reshape(modeImage,-1)
    bitStream = dataToBitArray(Data)
    # Adding 00111111111111110000 after data bits
    ...
    for i in range(0,length, step):
        for j in range(0,width, step):
            ...
            for x in range(step):
                for y in range(step):
                    element = modeImage[i+x][j+y]
                    modeDict[element] = modeDict[element]+1 if (element in modeDict) else 1
            
            mode = -1
            for modes in modeDict:
                if modeDict[modes] > modeDict[mode]:
                    mode = modes
            auxModes.append(mode)
            bitStream += intToBitArray(mode)
            for x in range(step):
                for y in range(step):
                    modeImage[i+x][j+y] -= mode
    # print(auxModes)
    return (modeImage, bitStream)
...
def turnToOriginal(modulatedImage, n, bitStream):
    ...
    for i in range(0,len(bitStream),16):
        modes.append(int(bitStream[i:i+16],2))
    # print(modes)
    step = 2*n+1
    ...
    for i in range(0,length, step):
        for j in range(0,width, step):
            referenceMode = modes[0]
            modes = modes[1:]
            for x in range(step):
                for y in range(step):
                    modulatedImage[i+x][j+y] += referenceMode
...
def starterFunction():
    ...
    modeImage, bitStream = modeify(img,n,data)
    copyOfMode = np.array(modeImage, dtype = np.uint8)
    print("\n\n----------------------------------------------------------\nModes subtracted")
    embeddedImage, minBin, maxBin = HistogramShift(modeImage, bitStream)
    copyOfEmbeddedImage = np.array(embeddedImage, dtype = np.uint8)
    print("\n\n----------------------------------------------------------\nHistogram Shifting Done")
    # Now we send all these to the reciever
    modeifiedImage, bitStream = reverseHS(embeddedImage, minBin, maxBin)
    print("\n\n----------------------------------------------------------\nReverse HS done")
    try:
        originalImage, data = turnToOriginal(modeifiedImage, n, bitStream)
    ...
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
            if copyOfImage[i][j] != originalImage[i][j]:
                a = False
                break
    p = Psnr(copyOfMode, copyOfEmbeddedImage)
    cv2.imshow("original-picture",copyOfImage)
    cv2.imshow("original-picture-mode-Subtracted",copyOfMode)
    cv2.imshow("embedded-picture-mode-Subtracted",copyOfEmbeddedImage)
